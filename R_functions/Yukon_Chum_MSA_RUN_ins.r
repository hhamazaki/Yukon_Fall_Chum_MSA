...
# Pilot Station Run  
Pilot_Run <- 'Daily Passage By Species.csv'
# Pilot Station Var  
Pilot_Var <- 'Daily Variance By Species.csv'
...
rstr <- read.csv(file.path(wd_Ins,strata_file),stringsAsFactors = FALSE)
...
# Read MSA prop table 
MSA <- read.csv(file.path(wd_Ins,stock_prop_file),stringsAsFactors = FALSE)
...
#  If PostSeason is TRUE, then read all historical Pilot data. 
   ...
   Pilot <- read.Pilot.ins.data(rstr,this.year) 
