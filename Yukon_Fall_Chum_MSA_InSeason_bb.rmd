...
# columns
n.col <- 5
  par(mfrow=c(n.row,n.col),mar = c(2,2,2,2),oma = c(3,3,3,3),yaxs='i',bty='l')
# yax: Index determining vertical axis needs to be labeled 
  yax <- c(1,which(1:(tny+1)%%n.row==0)+1) 
# sax: Index determining horizontal axis needs to be labeled  
  xax <- tny-n.col+(tny)%%n.col
  for(i in 1:tny){
    temp <- with(hsft, hsft[Year==tyears[i],])
    plot(Summer~stbreak, type ='o',col=4, xlim=c(1,9),ylim=c(0,100), 
         yaxt='n',xaxt='n',lwd = 2, data=temp,main=tyears[i])
