...
Pilot.d.min.max <- read.csv(file.path(wd_Sum,paste0('Pilot_d_min_max_',this.year,'.csv')),stringsAsFactors = FALSE)

# Read MSA Strata data
Pilot.hsft <- read.csv(file.path(wd_Sum,paste0('Pilot_sft_',this.year,'.csv')),stringsAsFactors = FALSE)
...
par(mfrow=c(1,1),mar = c(2,2,2,2),oma = c(3,3,3,3),yaxs='i',bty='l',las=1)
  ...
  plot(Run~Date, type='h',lwd=4,col='gray',data=Pilot[Pilot$Year==this.year,],xlim=xlims,xaxt='n')
...
par(mfrow=c(1,1),mar = c(2,2,2,2),oma = c(3,3,3,3),yaxs='i',bty='l',las=1)
  ...
  inc <- median(with(Pilot,tapply(Run,Year,max,na.rm=TRUE)))
  ry <-c(0,inc*ny)
  cl <- rainbow(ny)
  rstr.y <- rstr[rstr$Year==this.year,]
  xlims<-c(min(Pilot$Date2),max(Pilot$Date2))
  plot(Run~Date, data=Pilot[Pilot$Year==this.year,],type = "n", ylim = ry,xlim=xlims,yaxt='n',xaxt='n', ylab='',xlab='')
  axis.Date(1, at = stbreak,format= "%b%d",cex=0.6)
    for(i in 1:ny){
    abline(h = (i-1)*inc, col = "grey",lty=3,lwd=0.5)
    axis(2, at = (i-1)*inc, labels = years[i])
    temp <- Pilot[Pilot$Year==years[i],]
    polygon(temp$Date2,
            temp$Run + (i-1)*inc,
            col = t.col(cl[i],50),border=NA)
  rstr.y <- rstr[rstr$Year==years[i],]
#   lines(rstr.y$Strata_Start_Date,rep(inc,dim(rstr.y)[1]),col=4,lwd=2)
#   line(v=max(rstr.y$Strata_End_Date),col=2,lwd=2)
         }    
mtext(paste("Sampling Strata"), side = 3, line = 0, outer = TRUE,cex=1.5)
#mtext('Number of Salmon', side = 2, line = 1.5, las=0, outer = TRUE,cex=1.5)
mtext("Date", side = 1, line = 1, outer = TRUE,cex=1.5)
...
#  cl <- rainbow(ny)
  rstr.y <- rstr[rstr$Year==this.year,]
#  pilot.dp[is.na(pilot.dp)] <- 0
  
for(j in 1:6){
  inc <- median(tapply(pilot.dp[,4+j],pilot.dp$Year,max),na.rm=TRUE)  
  ry <-c(0,inc*ny)    
par(mfrow=c(1,1),mar = c(2,2,2,2),oma = c(3,3,3,3),yaxs='i',bty='l',las=1)
    plot(Run~Date, data= pilot.dp[ pilot.dp$Year==this.year,],type = "n", ylim = ry,xlim=xlims,yaxt='n',xaxt='n', ylab='',xlab='')
  axis.Date(1, at = stbreak,format= "%b%d",cex=0.6)
    for(i in 1:ny){
    abline(h = (i-1)*inc, col = "grey",lty=3,lwd=0.5)
    axis(2, at = (i-1)*inc, labels = years[i])
    temp <- pilot.dp[pilot.dp$Year==years[i],]
#    polygon(temp$Date2,temp[,4+j] + (i-1)*inc,col = t.col(j,50),border=NA)
    lines(temp$Date2,temp[,4+j] + (i-1)*inc,col = j)    
    rstr.y <- rstr[rstr$Year==years[i],]
#   lines(rstr.y$Strata_Start_Date,rep(inc,dim(rstr.y)[1]),col=4,lwd=2)
#   line(v=max(rstr.y$Strata_End_Date),col=2,lwd=2)
         }    
mtext(paste("Stock:",gname[j]), side = 3, line = 0, outer = TRUE,cex=1.5)
mtext("Date", side = 1, line = 1, outer = TRUE,cex=1.5)
 } 
```
**Figure 1.a.  Daily sonar counts attributed to chum salmon across all years, project operated near Pilot Station, Yukon Area.**
...
plot(order(temp1$Strata),type='n', xlim=c(1,8),ylim=c(0,100), ylab='Stock Percent',xlab='Sample Strata Number')
for (i in 1:6){
  lines(percent~Strata, type ='o',lwd=2,col=i,pch=i,data=Pilot.stpl[Pilot.stpl$group==ststockID[i],])
}
title(main = this.year)
legend('top',legend = gname,col = c(1:6), pch=c(1:6),lty=1,lwd=2,bty = 'n')
...
```{r,fig.width=9, fig.height = 5,echo=FALSE,dpi=400}
Pilot.stn <- Pilot.m[Pilot.m$Year == this.year,c('Year','Strata','Run', c('4','7','8','11','19','10'))]
gname <- lgnames[c('4','7','8','11','19','10')]      
...
sd <- Pilot.sd[Pilot.sd$Year==this.year,]
for(j in 1:8){
  ...
  lines(percent~stbreak,lwd=2,col=2, data=sd[sd$group ==fig.stock.id[j],])
  }
...
```{r,fig.width=9, fig.height = 9,echo=FALSE,dpi=400}
  par(mfrow=c(6,5),mar = c(2,2,2,2),oma = c(3,3,3,3),yaxs='i',bty='l') 
