...
if(dim(Pilot.fall)[1]>0){
  ...
  pieorder <- c(4,7,8,11,16,10,13)
  temp <- temp [match(pieorder,temp$grpID),]  
temp$pct <- with(temp, paste0(round(100*p),'%'))
par(mfrow=c(1,1),mar = c(4,4,4,4),oma = c(3,3,3,3))
pie(temp$p, init.angle=90,labels = paste(temp$GroupName,temp$pct),col=c(2:8),main="Stock Proportion (July 19 to End of Season)")
#legend("topright", paste(temp$GroupName,temp$pct),cex = 1,bty='n',fill =c(2:8))
#par(new=TRUE)
